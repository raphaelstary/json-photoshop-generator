...
function transformPSDoc(psDocument) {
    ...
    function parsePositionGroup(layerGroup) {
        return {
            type: 'group',
            drawables: parseLayerGroup(layerGroup),
            tags: parseTags(layerGroup.name.split(' '))
        };
    }
    ...
    function parseTaggedLayerGroup(layerGroup) {
        ...
        if (includes(layerGroup.name, '#button')) {
            ...
            drawables.push(parsePositionGroup(layerGroup));
