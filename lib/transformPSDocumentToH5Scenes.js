...
function transformPSDoc(psDocument) {
    ...
    function parseTaggedLayerGroup(layerGroup, animations) {
        ...
        if (includes(layerGroup.name, '#button')) {
            ...
            parseLayerGroup(layerGroup, animations).forEach(function (elem) {
                if (elem.tags) {
    ...
    function parseLayerGroup(layerGroup, animations) {
        ...
        layerGroup.layers.forEach(function (layer) {
            if (layer.type == 'layer' || layer.type == 'textLayer' || layer.type == 'shapeLayer') {
                drawables.push(parseLayer(layer, animations));
            ...
            if (layer.artboard) {
                ...
                layer.layers.forEach(function (layer) {
                    if (layer.type == 'layerSection' && startsWith(layer.name, 'layer')) {
                        currentScene[layer.name] = parseLayerGroup(layer, animations);
                    } // else skip
                });
