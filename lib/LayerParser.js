...
var LayerParser = (function (Vectors, StringUtils, rgbToHex, Layer, Tagger, Math) {
    ...
    LayerParser.prototype.parseBasicImage = function (layer) {
        ...
        if (layer.path && layer.path.pathComponents[0].origin.type == 'rect') {
            var mask = {};
            ...
            mask.x = Math.floor(bounds.left + width / 2 - this.doc.getArtboard().left);
            mask.y = Math.floor(bounds.top + height / 2 - this.doc.getArtboard().top);
