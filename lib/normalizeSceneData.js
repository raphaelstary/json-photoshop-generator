var normalizeSceneData = (function (Object) {
    ...
    function extractRotation(transform, returnObject) {
        var rotation = [];
        transform.forEach(function (frame) {
    ...
    function extractMaskScale(transform, base, returnObject) {
        var scale = [];
        transform.forEach(function (frame) {
            ...
            if (frame.width != undefined && frame.height != undefined)
                scale.push({
                    time: frame.time,
                    scale: Math.round(frame.width / base.width * 100) / 100
                });
        });
    ...
    function normalizeSceneData(h5Document, keyFrameResult, frameData) {
        ...
        Object.keys(keyFrameResult.animations).forEach(function (id) {
            ...
            if (wrapper.mask) {
                var mask = {};
                extractPosition(wrapper.mask, mask);
                extractRotation(wrapper.mask, mask);
                extractMaskScale(wrapper.mask, drawable.mask, mask);
